...
class TreeNodeHelper {
    ...
    //传入普通节点，转换成排序后的Node
    func getSortedNodes(_ groups: [TreeNode], defaultExpandLevel: Int) -> [TreeNode] {
        var result: [TreeNode] = []
       //result.removeAll()
    ...
    //将数据转换成书节点
    func convetData2Node(_ nodes: [TreeNode]) -> [TreeNode] {
       // var nodes: [TreeNode] = []
        ...
        for i in 0 ..< nodes.count {
            ...
            for j in i+1 ..< nodes.count {
                ...
                if m.pId == n.id {
                    ...
                    if(n.children.contains{ $0 === m } ==  false)
                    {
                       // print("c")
                        n.children.append(m)
                         m.parent = n
                    ...
                    if(m.children.contains{ $0 === n } ==  false)
                    {
                       m.children.append(n)
                       n.parent = m
                    }
